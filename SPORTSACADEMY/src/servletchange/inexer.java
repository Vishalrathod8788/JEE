...
@WebServlet("/inexer")
public class inexer extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try
		{
			...
			String en,gn,enam;
			int se,re;
			enam=request.getParameter("exn");
			en=enam.toUpperCase();
			gn=request.getParameter("gnam");
			
			se=Integer.parseInt(request.getParameter("set"));
			re=Integer.parseInt(request.getParameter("rep"));
			stmt=conn.createStatement();
			qry="select * from exercise where exercisename='"+en+"' and gamename='"+gn+"'";
			...
			if(rs.next())
			{
				...
				response.sendRedirect("invexer.html");
				
			}
			else
			{
				//response.sendRedirect("fetchtab");
				String sql="{CALL myexinsert(?,?,?,?)}";
