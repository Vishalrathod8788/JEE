...
@WebServlet("/redirectdiet")
public class redirectdiet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(re.next())
		{
			...
			{
				response.sendRedirect("cricdiet2.html");
			}
			else if(gn.equals("KABBADI"))
			{
				response.sendRedirect("kabdiet2.html");
			}
			else if(gn.equals("TENNIS"))
			{
				response.sendRedirect("tendiet2.html");
			}
			else if(gn.equals("BADMINTON"))
			{
				response.sendRedirect("baddirt2.html");
			}
			else if(gn.equals("VOLLEYBALL"))
			{
				response.sendRedirect("voldiet2.html");
			}
