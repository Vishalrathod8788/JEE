...
public class getstaff extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			qry="select * from staff where sno="+id+"";
			...
			pw.println("<form action='upstaff' method='post'>");
					...
					else
					{
						...
						pw.println(" <input type='radio'  name='gen' value='MALE'  required>MALE");		  
								pw.println("<input type='radio'  name='gen' value='FEMALE' checked required>FEMALE");
										...
										pw.println("<input type='text' name='dep' value="+rs.getString(7)+" required>");	
					
										pw.println("<label>Post :</label>");		
										pw.println("<input type='text' name='po' value="+rs.getString(8)+" required>");	
					
					pw.println("<label>Qualification :</label>");		
					pw.println("<input type='text' name='ql' value="+rs.getString(9)+" required>");
								
					pw.println("<label>Experience  :</label>");
					pw.println("<input type='text' name='ex' value="+rs.getString(10)+"required>");	
					
					pw.println("<label>contact no  :</label>");
					pw.println("<input type='text' name='cnum' value="+rs.getString(11)+"required>");	
